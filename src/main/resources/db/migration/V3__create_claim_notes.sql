-- claim_notes: synthetic adjuster note source.
-- ingested = false acts as a watermark; the notes-sync adapter reads false rows,
-- ingests them through the same IngestionService port, then flips to true.
...
-- Seed data: 6 notes × 4 claims = 24 rows, all ingested = false
